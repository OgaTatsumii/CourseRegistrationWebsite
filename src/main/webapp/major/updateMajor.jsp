...
<title>Admin</title>
<link rel="stylesheet" href="css/style009x`.css" />
				...
				<div class="addStudentTitle" style="display: flex;">
					...
					<div id="toast"></div>
						...
						<div class="form-content">
							...
							<div class="form-group">
									<div class="idNganh">
										<label for="MaNganh">Mã Ngành :</label> <input
											...
											value="<%=(Nganh != null) ? Nganh.getMaNganh() : maNganh%>"
											required />
 										<p class="error">
											<%=errorMessageMaNganh%>
										</p> 
								...
								<div class="form-group">
									<div class="tenNganh">
										<label for="TenNganh">Tên Ngành :</label> <input
											...
											value="<%=(Nganh != null) ? Nganh.getTenNganh() : tenNganh%>"
											required />
										<p class="error">
											<%=errorMessageTenNganh%>
										</p>
								...
								<div class="form-group">
									<div class="gia1tinchi">
										<label for="GiaMotTinChi">Giá một tín chỉ :</label> <input
											...
											value="<%=(Nganh != null) ? Nganh.getGiaMotTinChi() : giaMotTinChi%>"
											required />
									</div>
								</div>
								<div class="form-group">
									<div class="SoTinChiNganh">
										<label for="SoTinChiNganh">Tổng số tín chỉ :</label> <input
											...
											value="<%=(Nganh != null) ? Nganh.getSoTinChi(): soTinChiNganh%>"
											required />
									</div>
								</div>
								<div class="form-group">
									<div class="thoiGianDaoTao">
										<label for="ThoiGianDaoTao">Thời gian đào tạo :</label> <input
											...
											value="<%=(Nganh != null) ? String.valueOf(Nganh.getThoiGianDaoTao()): thoiGianDaoTao%>"
											required />
									</div>
								</div>
								<div class="form-group">
									<div class="maKhoa">
										<label for="MaKhoa">Khoa :</label> <select
										 id="MaKhoa" name="MaKhoa" required >
											<%
											ArrayList<Khoa> danhSachKhoas = (ArrayList<Khoa>) session.getAttribute("dsKhoa");
											if (danhSachKhoas != null) {
												for (Khoa Khoa : danhSachKhoas) {
											%>
											<option value="<%= (Nganh != null ? Khoa.getMaKhoa() : maKhoa) %>"
    										<%= (Nganh != null && Nganh.getKhoa() != null && Khoa.getMaKhoa().equals(Nganh.getKhoa().getMaKhoa()))
    										|| (Nganh == null && maKhoa.equals(Khoa.getMaKhoa()))? "selected" : "" %>>
    										<%= Khoa.getTenKhoa() %>
											</option>										
											<%
											}
							...
							<button class="addSubject" type="submit">Sửa Ngành Học</button>
						</form>
						</div>
					</div>
